...
# Updated to use a unique title to avoid duplication errors and set goal to 20000
CAMPAIGN = {
    "title": "Help Aya and her family survive the bombing - Urgent Appeal",
...
def main():
    ...
    js3_goal = f"""
    (function() {{
         // Goal logic would go here if specific selector known. For now relying on default or manual adjust later if field not standard. 
         // Assuming goal input might be present.
         // Actually, let's check for goal input.
         const g = document.querySelector('input[type="number"]') || document.querySelector('input[placeholder*="Goal"]');
         if (g) {{
             g.value = {CAMPAIGN['goal']};
    ...
    """
    # Note: Goal is usually in step 3 or 4.
