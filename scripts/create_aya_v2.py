...
def run_js(ws, js, await_promise=False):
    msg = json.dumps({
...
def main():
    ...
    (async function() {
        ...
        if (addr) {
            ...
            // Simulate enter or selection if needed, but usually input is enough for validation if valid string
        } else {
        ...
        const btn = document.getElementById('saveStep1') || document.querySelector('button.mat-mdc-button-base'); 
        // Note: saveStep1 ID observed in logs.
        if (btn) {
            btn.disabled = false;
            btn.click();
            return "NEXT_CLICKED";
    ...
    """
    res1 = run_js(ws, js1, await_promise=True)
    ...
    """
    res2 = run_js(ws, js2, await_promise=True)
